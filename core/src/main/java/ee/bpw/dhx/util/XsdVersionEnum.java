...
public enum XsdVersionEnum {
  ...
  public static XsdVersionEnum forClass(Class<? extends Object> containerClass) throws DhxException{
    if (containerClass != null) {
      for (XsdVersionEnum version : XsdVersionEnum.values()) {
        if (containerClass.equals(version.getContainerClass())) {
          log.debug("Found XSD version for class. containerClass:" + containerClass.toString() + " version:" + version.toString());
          return version;
        }
      }
      throw new DhxException(DhxExceptionEnum.TECHNICAL_ERROR, "Unknown class. No XSD version is found for that class. Class:" +containerClass.toString());
    }
    return null;
  }
