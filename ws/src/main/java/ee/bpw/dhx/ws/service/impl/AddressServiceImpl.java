...
public class AddressServiceImpl implements AddressService{
	...
	/**
	 * Methods need to be overriden if in memmory address list os not an option. Then getAddresseeList might fetch address list from DB for example
	 */
	...
	/**
	 * Methods need to be overriden if in memmory address list os not an option. Then setAddresseeList might save addresses to DB for example
	...
	/**
	 * Postconstruct method. Refreshes address list at startup. 
	 * Override might be needed if need to read from DB  and no need to refresh whole list  at startup for example.
	 */
	@PostConstruct
	public void init() {
		renewAddressList();
	...
	private List<XroadMember> getRepresentees (XroadMember member) throws DhxException{
		...
		if(response == null || response.getMembers() == null || response.getMembers().getMember() == null || response.getMembers().getMember().size()==0) {
			return null;
		} else {
	...
	public XroadMember getClientForMemberCode(String memberCode) throws DhxException{
		...
		for(XroadMember member : members) {
			if (member.getMemberCode().equals(memberCode) && (member.getRepresentee()==null || member.getRepresentee().getMemberCode()==null)){
				return member;
			} else if (member.getRepresentee() != null && member.getRepresentee().getMemberCode().equals(memberCode)
					 && (member.getRepresentee().getStartDate().getTime() <= curDate.getTime()
					 && (member.getRepresentee().getEndDate() == null || member.getRepresentee().getEndDate().getTime() >= curDate.getTime())) ) {
				return member;
			}
		}
